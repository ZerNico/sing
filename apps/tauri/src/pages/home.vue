...
const cards: {
  ...
  },
  {
    ...
    action: () => waitForExit(() => router.push({ name: '/lobby/' })),
    animation: 'pulse',
  ...
  },
  {
    ...
    action: () => waitForExit(() => router.push({ name: '/settings/' })),
    animation: 'spin',
    description: 'Change your settings or add your songs and microphones.',
  },
]

const waitForExit = async (fn: (...args: any[]) => any) => {
  await animation.exit()
  fn()
}
...
const animation = usePageAnimation({ duration: 500 })
</script>

<template>
  <div class="h-full w-full">
    <Layout class="gradient-bg-main" :scrolling="false">
      <template #header>
      ...
      <div class="flex flex-col items-center justify-center py-5cqh h-full">
        ...
        <div
          class="h-4/7 w-full flex gap-1cqw transform transition-all duration-1000"
          :class="{ 'translate-y-60cqh': animation.animated.value }"
        >
          <ModeCard
      ...
      <template #footer>
        <KeyHints :hints="['navigate', 'confirm']" :animated="animation.animated.value" />
      </template>
    </Layout>
  </div>
</template>
