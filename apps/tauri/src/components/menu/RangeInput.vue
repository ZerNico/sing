  ...
  <div class="relative py-2.1cqh select-none">
    <div class="relative z-1 flex items-center gap-1cqh px-10cqw">
      ...
      <div class="flex items-center gap-2cqh">
        <Icon role="button" icon="TriangleArrow" class="rotate-90 transform h-2.8cqh w-auto" @click="onClick('left')" />
        <div class="relative h-2.8cqh w-50cqw">
          ...
          />
          <div
            class="absolute inset-0 bg-black/40 pointer-events-none rounded-0.3cqw flex items-center justify-center font-semibold text-2cqh"
          >
            {{ modelValue }}
          </div>
          <div
            class="absolute inset-0 bg-white pointer-events-none rounded-0.3cqw bar flex items-center justify-center font-semibold text-2cqh text-black"
          >
            {{ modelValue }}
          </div>
        </div>
...
<style scoped>
.bg {
  background: linear-gradient(90deg, v-bind('props.gradient.start') 0%, v-bind('props.gradient.end') 100%);
}
