...
<template>
  <div class="flex flex-col gap-2" role="button">
    ...
    >
      <div class="banner flex justify-center items-center rounded-0.4cqw transition-all duration-300">
        <Icon v-if="!loading" class="text-white !w-1/2 !h-1/2 block" :icon="icon" />
        <Icon v-else class="text-white !w-1/2 !h-1/2 block animate-spin" icon="Spinner" />
      </div>
    </div>
  </div>
</template>
